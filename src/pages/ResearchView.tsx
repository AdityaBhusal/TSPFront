...
import type { AlgoKey, Pin, Matrix, RouteResult } from '../App'

export function ResearchView() {
  const [pins, setPins] = useState<Pin[]>([])
  const [algos, setAlgos] = useState<AlgoKey[]>(['nearest_neighbor'])
  const [benchmarkMode, setBenchmarkMode] = useState(false)
  const [enableRoadSnapping, setEnableRoadSnapping] = useState(false)
  ...
  function handleAddRandomPoints(count: number, bounds: { minLat: number; maxLat: number; minLng: number; maxLng: number }) {
    ...
    for (let i = 0; i < count; i++) {
      // Generate points within a circle using polar coordinates
